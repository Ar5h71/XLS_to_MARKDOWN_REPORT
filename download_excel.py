...
DEFAULT_FIELDS_FROM_ALL_VERSIONS = "false"
...
for i in range(len(ASSET)):
    if __name__ == "__main__":
        ...
        create_export(ASSET[i],USERNAME,PASSWD,DOMAIN)
        list_exports(ASSET[i],USERNAME,PASSWD,DOMAIN)
        url.append(latest_url(ASSET[i],USERNAME,PASSWD,DOMAIN))

for i in range(len(ASSET)):
    resp = requests.get(url[i], auth=(USERNAME, PASSWD))
    url2 = url[i][66:]
    url2 = url2.replace('/','_')
    url2 = url2.replace('%20','_')
    with open(url2, 'wb') as output:
